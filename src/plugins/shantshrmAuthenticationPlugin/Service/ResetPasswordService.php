...
use Exception;
use ShantsHRM\Admin\Dto\UserSearchFilterParams;
use ShantsHRM\Admin\Traits\Service\UserServiceTrait;
use ShantsHRM\Authentication\Dao\ResetPasswordDao;
...
use ShantsHRM\Core\Utility\Base64Url;
use ShantsHRM\Entity\Employee;
use ShantsHRM\Entity\EmployeeTerminationRecord;
use ShantsHRM\Entity\ResetPasswordRequest;
...
class ResetPasswordService
{
    ...
    public function generateEmailBody(string $templateFile, array $placeholders, array $replacements)
    {
        $body = file_get_contents(
            Config::get(
                Config::PLUGINS_DIR
            ) . '/shantshrmAuthenticationPlugin/config/data/' . $templateFile
        );
    ...
    public function sendPasswordResetCodeEmail(Employee $receiver, string $resetCode, string $userName): bool
    {
        try {
            ...
            $this->getEmailService()->setMessageFrom(
                [$this->getEmailService()->getEmailConfig()->getSentAs() => 'ShantsHRM']
            );
            $this->getEmailService()->setMessageSubject('ShantsHRM Password Reset');
