...
use ShantsHRM\Authentication\Exception\SessionExpiredException;
use ShantsHRM\Authentication\Exception\UnauthorizedException;
use ShantsHRM\Core\Controller\AbstractViewController;
...
class AuthenticationSubscriber extends AbstractEventSubscriber
{
    ...
    public function onControllerEvent(ControllerEvent $event): void
    {
        ...
        if ($this->getControllerInstance($event) instanceof AbstractRestController) {
            ...
            $response->setContent(
                \ShantsHRM\Core\Api\V2\Response::formatError(
                    ['error' => ['status' => Response::HTTP_UNAUTHORIZED, 'message' => $message]]
                )
            );
            $response->setStatusCode(Response::HTTP_UNAUTHORIZED);
            $response->headers->set(
                \ShantsHRM\Core\Api\V2\Response::CONTENT_TYPE_KEY,
                \ShantsHRM\Core\Api\V2\Response::CONTENT_TYPE_JSON
            );
