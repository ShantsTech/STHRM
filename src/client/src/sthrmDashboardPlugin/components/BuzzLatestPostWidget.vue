...
<template>
  ...
  >
    <oxd-grid class="sthrm-buzz-widget" :cols="1">
      <oxd-grid-item
        v-for="post in posts"
        :key="post"
        class="sthrm-buzz-widget-card"
      >
        <div class="sthrm-buzz-widget-header" @click="onClickPost">
          <profile-image :employee="post.employee"></profile-image>
          <div class="sthrm-buzz-widget-header-text">
            <oxd-text tag="p" class="sthrm-buzz-widget-header-emp">
              {{ post.employeeFullName }}
            </oxd-text>
            <oxd-text tag="p" class="sthrm-buzz-widget-header-time">
              {{ post.dateTime }}
            </oxd-text>
          </div>
        </div>
        <oxd-divider class="sthrm-buzz-widget-divider" />
        <oxd-text v-if="post.text" tag="p" class="sthrm-buzz-widget-body">
          {{ post.text }}
        </oxd-text>
        <img
          v-if="post.postImgSrc"
          :src="post.postImgSrc"
          class="sthrm-buzz-widget-picture"
        />
...
import ProfileImage from '@/sthrmBuzzPlugin/components/ProfileImage.vue';
