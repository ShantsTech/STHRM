...
<template>
  <oxd-sheet :gutters="false" class="sthrm-dashboard-widget">
    <div class="sthrm-dashboard-widget-header">
      <div class="sthrm-dashboard-widget-name">
        <oxd-icon
          :name="icon"
          :type="iconType"
          class="sthrm-dashboard-widget-icon"
        ></oxd-icon>
        <oxd-text tag="p">
          {{ title }}
        </oxd-text>
    ...
    <oxd-loading-spinner
      v-if="loading"
      class="sthrm-dashboard-widget-loader"
    />
    <div v-else ref="widgetRef" :class="widgetBodyClasses">
      <slot></slot>
      <div v-if="empty" class="sthrm-dashboard-widget-body-nocontent">
        <img
          :src="defaultPic"
          alt="No Content"
          class="sthrm-dashboard-widget-img"
        />
        <oxd-text tag="p">
...
export default {
  ...
  setup() {
    ...
    const widgetBodyClasses = computed(() => ({
      'sthrm-dashboard-widget-body': true,
      '--scroll-visible':
