...
<template>
  <oxd-form class="sthrm-paper-container">
    <div class="sthrm-timesheet-header">
      <div class="sthrm-timesheet-header--title">
        <slot name="header-title"></slot>
      </div>
      <div class="sthrm-timesheet-header--options">
        <slot name="header-options"></slot>
      </div>
    </div>

    <div v-if="loading" class="sthrm-timesheet-loader">
      <oxd-loading-spinner />
    </div>
    <div
      v-else-if="!loading && !columns"
      class="sthrm-timesheet-body-message"
    >
      <oxd-alert
        type="warn"
        :show="true"
    ...
    <div v-else class="sthrm-timesheet-body">
      <table :class="tableClasses">
        <thead class="sthrm-timesheet-table-header">
          <tr class="sthrm-timesheet-table-header-row">
            <th :class="fixedColumnClasses">
              {{ $t('time.project') }}
            </th>
            <th class="sthrm-timesheet-table-header-cell">
              {{ $t('time.activity') }}
            </th>
            ...
            <th
              ...
              :key="day.id"
              class="sthrm-timesheet-table-header-cell --center"
            >
              <span class="--day">
                {{ day.day }}
              </span>
            ...
            <th
              v-if="!editable"
              class="sthrm-timesheet-table-header-cell --center --freeze-right"
            >
              {{ $t('general.total') }}
            </th>
          </tr>
        ...
        <tbody class="sthrm-timesheet-table-body">
          <!-- timesheet activities -->
          <tr
            v-for="(record, i) in records"
            :key="record"
            class="sthrm-timesheet-table-body-row"
          >
            ...
            <td class="sthrm-timesheet-table-body-cell">
              <activity-dropdown
                v-if="editable"
            ...
            <td
              ...
              :class="{
                'sthrm-timesheet-table-body-cell': true,
                '--center': true,
            ...
            >
              <oxd-icon-button
                ...
                class="sthrm-timesheet-icon-comment"
            ...
            <td
              v-if="!editable"
              class="sthrm-timesheet-table-body-cell --center --freeze-right --highlight"
            >
              {{ record.total.label }}
            </td>
            <td
              v-if="editable"
              class="sthrm-timesheet-table-body-cell --flex"
            >
              <oxd-icon-button
                name="trash"
                class="sthrm-timesheet-icon"
                @click="deleteRow(i)"
              />
            </td>
          </tr>
          ...
          <tr
            ...
            class="sthrm-timesheet-table-body-row --total"
          >
            <td
              class="sthrm-timesheet-table-body-cell --freeze-left --highlight"
            >
              {{ $t('general.total') }}
            </td>
            <td></td>
            ...
            <td
              ...
              :key="`total-${date}`"
              class="sthrm-timesheet-table-body-cell --center"
            >
              {{ date.total.label }}
            </td>
            <!-- total per day -->
            <td
              class="sthrm-timesheet-table-body-cell --center --freeze-right --highlight-2"
            >
              {{ subtotal }}
            </td>
          </tr>
          ...
          <!-- add row -->
          <tr v-if="editable" class="sthrm-timesheet-table-body-row">
            <td class="sthrm-timesheet-table-body-cell --flex">
              <oxd-icon-button
                name="plus"
                class="sthrm-timesheet-icon"
                @click="addRow"
          ...
          <tr
            ...
            class="sthrm-timesheet-table-body-row"
          >
            <td colspan="9" class="sthrm-timesheet-table-body-cell">
    ...
    <div class="sthrm-timesheet-footer">
      <div class="sthrm-timesheet-footer--title">
        <slot name="footer-title"></slot>
      </div>
      <div class="sthrm-timesheet-footer--options">
...
import ActivityDropdown from '@/sthrmTimePlugin/components/ActivityDropdown.vue';
import ProjectAutocomplete from '@/sthrmTimePlugin/components/ProjectAutocomplete.vue';
import TimesheetCommentModal from '@/sthrmTimePlugin/components/TimesheetCommentModal.vue';
...
export default {
  ...
  computed: {
    ...
    tableClasses() {
      return {
        'sthrm-timesheet-table': true,
        '--editable': this.editable,
      };
    },
    fixedColumnClasses() {
      return {
        'sthrm-timesheet-table-header-cell': true,
