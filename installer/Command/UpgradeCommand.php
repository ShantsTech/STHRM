...
use ShantsHRM\Installer\Controller\Upgrader\Api\ConfigFileAPI;
use ShantsHRM\Installer\Controller\Upgrader\Api\UpgraderDataRegistrationAPI;
...
class UpgradeCommand extends InstallerCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $this->getIO()->caution(
            "IMPORTANT: Make sure it's a copy of the database of your current ShantsHRM installation and not the original database. It's highly discouraged to use the original database for upgrading since it won't be recoverable if an error occurred during the upgrade."
        );
        $this->getIO()->warning(
        ...
        $this->getIO()->block(
            'Select your current ShantsHRM version here. You can find the version at the bottom of the ShantsHRM login page. ShantsHRM Upgrader only supports versions listed in the dropdown. Selecting a different version would lead to an upgrade failure and a database corruption.'
        );
        ...
        if ($input->isInteractive() && $currentVersion == null) {
            $question = new ChoiceQuestion('Current ShantsHRM Version ' . self::REQUIRED_TAG, $versions);
        ...
        $this->getIO()->title('Upgrading ShantsHRM');
        $fromAndToVersions = "from <comment>ShantsHRM $currentVersion</comment> to <comment>ShantsHRM " . Config::PRODUCT_VERSION . '</comment>';
        ...
        if (!$input->isInteractive()) {
            $this->getIO()->info(
                "Upgrading from ShantsHRM $currentVersion to ShantsHRM " . Config::PRODUCT_VERSION . '.'
            );
        }
