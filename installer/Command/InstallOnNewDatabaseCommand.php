...
use ShantsHRM\Installer\Controller\Installer\Api\ConfigFileAPI;
use ShantsHRM\Installer\Controller\Installer\Api\InstallerDataRegistrationAPI;
use ShantsHRM\Installer\Exception\InterruptProcessException;
use ShantsHRM\Installer\Exception\InvalidArgumentException;
use ShantsHRM\Installer\Framework\InstallerCommand;
use ShantsHRM\Installer\Util\AppSetupUtility;
use ShantsHRM\Installer\Util\Connection;
use ShantsHRM\Installer\Util\DatabaseUserPermissionEvaluator;
...
class InstallOnNewDatabaseCommand extends InstallerCommand
{
    ...
    public const STEP_5 = 'Create ShantsHRM database user';
    ...
    protected function licenseAcceptance(): void
    {
        ...
        $this->getIO()->block('Please review the license terms before installing ShantsHRM Starter.');
    ...
    protected function databaseInformation(): void
    {
        ...
        $useSameDbUser = $this->getIO()->confirm(
            'Use the same `Privileged Database User` as `ShantsHRM Database User`',
            false
        );
        ...
        if ($useSameDbUser === false) {
            $ohrmDbUser = $this->getRequiredField('ShantsHRM Database Username');
            $ohrmDbPassword = $this->getIO()->askHidden(
                'ShantsHRM Database User Password <comment>(hidden)</comment>'
            );
        }
        ...
        if (!$useSameDbUser && $this->getAppSetupUtility()->isDatabaseUserExist($ohrmDbUser)) {
            $this->getIO()->error(
                "Database User `$ohrmDbUser` Already Exist. Please Use Another Username for `ShantsHRM Database Username`."
            );
            StateContainer::getInstance()->clearDbInfo();
            goto dbInfo;
        }
    ...
    protected function instanceCreation(): void
    {
        ...
        $this->getIO()->block(
            'Fill in your organization details here. Details entered in this section will be captured to create your ShantsHRM Instance'
        );
    ...
    protected function adminUserCreation(): void
    {
        ...
        $regConsent = $this->getIO()->confirm(
            'Register your system with ShantsHRM. By registering, You will be eligible for free support via emails, receive security alerts and news letters from ShantsHRM.',
            true
        );
        StateContainer::getInstance()->storeRegConsent($regConsent);

        $this->getIO()->note(
            'Users who seek access to their data, or who seek to correct, amend, or delete the given information should direct their requests to data@hrm.shants-tech.com'
        );
    }
