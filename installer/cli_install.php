...
if ($cliConfig['license']['agree'] != 'y') {
    ...
    echo "For continue installation need to accept ShantsHRM license agreement. It is available in '$licenseFilePath'.";
    die;
}
...
$useSameDbUserForShantsHRM = $cliConfig['database']['useSameDbUserForShantsHRM'] == 'y';
...
if ($dbType === AppSetupUtility::INSTALLATION_DB_TYPE_NEW) {
    ...
    if (!$useSameDbUserForShantsHRM) {
        $ohrmDbUser = $cliConfig['database']['shantshrmDatabaseUser'];
        $ohrmDbPassword = $cliConfig['database']['shantshrmDatabasePassword'];
    }
...
echo "Create ShantsHRM database user\n";
