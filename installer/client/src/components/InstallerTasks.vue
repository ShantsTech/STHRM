 ...
 */
 -->
<template>
  <div class="shantshrm-installer-task">
    <div
      v-for="task in tasks"
      :key="task"
      class="shantshrm-installer-task-item"
    >
      <oxd-text
        tag="p"
        :class="{
          'shantshrm-installer-task-item-name': true,
          '--active': task.state === 1,
          '--error': task.state === 3,
        }"
      >
      ...
      <div class="shantshrm-installer-task-item-progress">
        <oxd-loading-spinner v-if="task.state === 1" :with-container="false" />
        <div
          v-else-if="task.state === 2"
          class="shantshrm-installer-task-icon --done"
        >
          <oxd-icon name="check" />
        </div>
        <div
          v-else-if="task.state === 3"
          class="shantshrm-installer-task-icon --error"
        >
          <oxd-icon name="exclamation" />
        </div>
        <div v-else class="shantshrm-installer-task-icon --pending">
          <oxd-icon name="dash" />
        </div>
      </div>
    </div>
